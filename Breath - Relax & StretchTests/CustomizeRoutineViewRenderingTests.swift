...
struct CustomizeRoutineViewRenderingTests {
    ...
    @Test func rendersWithExercises() {
        ...
        let view = CustomizeRoutineView(title: "Wake Up", exercises: exercises, isPinned: false, onDone: { _, _, _, _ in })
    ...
    @Test func rendersWithPinnedStateOn() {
        ...
        let view = CustomizeRoutineView(title: "Wake Up", exercises: exercises, isPinned: true, onDone: { _, _, _, _ in })
            .environmentObject(ExercisePickingSession())
        let renderer = ImageRenderer(content: view.frame(width: 390, height: 700))
        #expect(renderer.cgImage != nil)
    }
    ...
    @Test func rendersWithNoExercises() {
        let view = CustomizeRoutineView(title: "Wake Up", exercises: [], isPinned: false, onDone: { _, _, _, _ in })
            .environmentObject(ExercisePickingSession())
        let renderer = ImageRenderer(content: view.frame(width: 390, height: 700))
        #expect(renderer.cgImage != nil)
    }

...
1. [x] **Provision the backend** — done, verified 2026-09-10 via Supabase MCP against project `wmsutfittuxrvcwuywrk`: all 4 tables exist with RLS enabled, the app's baked-in publishable key matches the project's actual key. Also found and fixed a real gap while verifying: the live DB still had pre-Apple-Sign-In permissive policies (`"anyone can upsert/update profiles"`, `"anyone can upsert/update routines"`, `"anyone can insert sessions"`, all with `using/check (true)`) coexisting with the correct `auth.uid()`-scoped ones — Postgres OR's multiple permissive policies together, so the open ones silently won. Dropped them (migration `drop_stale_permissive_write_policies`); `supabase_schema.sql` updated to match the live policy set exactly. **Still open, needs Jason in the dashboard:** confirm the Apple auth provider is actually enabled (Authentication → Providers) — not checkable via SQL/MCP, and if it isn't, every Apple sign-in token exchange 4xxs silently (app just stays local).
